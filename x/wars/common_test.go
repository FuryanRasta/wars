...
func newValidMsgCreateSwapperWar() types.MsgCreateWar {
...
func newValidMsgCreateAugmentedWar() types.MsgCreateWar {
	validMsg := newValidMsgCreateWar()
...
func newValidMsgCreateWar() types.MsgCreateWar {
	functionType := types.PowerFunction
	functionParams := functionParametersPower()
	reserveTokens := powerReserves()
	return types.NewMsgCreateWar(token, initName, initDescription, initCreator,
...
func addCoinsToUser(app *simapp.SimApp, ctx sdk.Context, coins sdk.Coins) error {
	_, err := app.WarsKeeper.BankKeeper.AddCoins(ctx, userAddress, coins)
	return err
}

func addCoinsToUser2(app *simapp.SimApp, ctx sdk.Context, coins sdk.Coins) error {
	_, err := app.WarsKeeper.BankKeeper.AddCoins(ctx, anotherAddress, coins)
	return err
}
