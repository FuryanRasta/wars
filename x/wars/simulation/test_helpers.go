...
func getNextWarName() string {
	return tokenPrefix + strconv.Itoa(totalWarCount+1)
}

func incrementWarCount() {
	totalWarCount += 1
}

func newSwapperWar(token string) {
	swapperWars = append(swapperWars, token)
}

func getRandomWarName(r *rand.Rand) (warName string, ok bool) {
	if totalWarCount == 0 {
		return "", false
	} else if totalWarCount == 1 {
		return tokenPrefix + "1", true
	} else {
		randInt := simulation.RandIntBetween(r, 1, totalWarCount)
		return tokenPrefix + strconv.Itoa(randInt), true
	}
}

func getRandomWarNameExcept(r *rand.Rand, except string) (warName string, ok bool) {
	if totalWarCount == 0 {
		return "", false
	} else if totalWarCount == 1 {
		...
		token := except
		for token == except {
			randInt := simulation.RandIntBetween(r, 1, totalWarCount+1)
...
func getInitialWarState(functionType string) string {
	switch functionType {
	case types.AugmentedFunction:
