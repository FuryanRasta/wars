...
func TestNewWarDefaultValuesAndSorting(t *testing.T) {
	...
	war := NewWar(initToken, initName, initDescription, initCreator,
	...
	expectedCurrentSupply := sdk.NewInt64Coin(war.Token, 0)

	require.Equal(t, expectedCurrentSupply, war.CurrentSupply)
	require.Equal(t, sortedReserveTokens, war.ReserveTokens)
	require.Equal(t, sortedOrderQuantityLimits, war.OrderQuantityLimits)
}

func TestGetNewReserveCoinReturnPasses(t *testing.T) {
	war := getValidWar()

	require.Equal(t, sdk.NewInt64Coin(war.Token, 0), war.CurrentSupply)
}

func TestGetNewReserveDecCoins(t *testing.T) {
	war := getValidWar()
	war.ReserveTokens = []string{"aaa", "bbb"}

	amount := sdk.MustNewDecFromStr("10")
	actualResult := war.GetNewReserveDecCoins(amount)
...
func TestGetPricesAtSupply(t *testing.T) {
	...
	for _, tc := range testCases {
		war.FunctionType = tc.functionType
		war.FunctionParameters = tc.functionParams
		war.ReserveTokens = tc.reserveTokens
		war.State = tc.state

		actualResult, err := war.GetPricesAtSupply(tc.supply)
...
func TestGetCurrentPrices(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		war.ReserveTokens = tc.reserveTokens
		war.State = tc.state
		war.CurrentSupply = sdk.NewCoin(war.Token, tc.currentSupply)

		actualResult, _ := war.GetCurrentPricesPT(tc.reserveBalances)
		expectedDec := sdk.MustNewDecFromStr(tc.expected)
...
func TestReserveAtSupply(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		actualResult := war.ReserveAtSupply(tc.supply)
...
func TestGetReserveDeltaForLiquidityDelta(t *testing.T) {
	war := getValidWar()
	war.FunctionType = SwapperFunction
	war.ReserveTokens = swapperReserves()
	// TODO: add more test cases
	...
	for _, tc := range testCases {
		...
		actualResult := war.GetReserveDeltaForLiquidityDelta(
...
func TestGetPricesToMint(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		actualResult, err := war.GetPricesToMint(tc.amount, tc.reserveBalances)
		if tc.fails {
			require.Error(t, err)
		} else {
			require.Nil(t, err)
...
func TestGetReturnsForBurn(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		actualResult := war.GetReturnsForBurn(tc.amount, tc.reserveBalances)
...
func TestGetReturnsForSwap(t *testing.T) {
	...
	for _, tc := range testCases {
		war.TxFeePercentage = sdk.MustNewDecFromStr(tc.warTxFee)
		fromAmount := sdk.NewCoin(tc.from, tc.amount)
		actualResult, actualFee, err := war.GetReturnsForSwap(
...
func TestGetReturnsForSwapNonSwapperFunctionFails(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		_, _, err := war.GetReturnsForSwap(dummyCoin, "", sdk.Coins{})
		require.Error(t, err)
	}
}

func TestWarGetTxFee(t *testing.T) {
	war := War{}
	...
	for _, tc := range testCases {
		...
		war.TxFeePercentage = tc.txFeePercentage
		require.Equal(t, expected, war.GetTxFee(inputToken))
	}
}

func TestWarGetExitFee(t *testing.T) {
	war := War{}
	zeroPointOne := sdk.MustNewDecFromStr("0.1")
	...
	for _, tc := range testCases {
		...
		war.ExitFeePercentage = tc.exitFeePercentage
		require.Equal(t, expected, war.GetExitFee(inputToken))
	}
}

func TestWarGetTxFees(t *testing.T) {
	war := War{}
	war.TxFeePercentage = sdk.MustNewDecFromStr("0.1")
	...
	require.Equal(t, expected, war.GetTxFees(inputTokens))
}

func TestWarGetExitFees(t *testing.T) {
	war := War{}
	...
	require.Equal(t, expected, war.GetExitFees(inputTokens))
}
...
func TestSignersEqualTo(t *testing.T) {
	...
	war.Signers = []sdk.AccAddress{addr1, addr2}
	...
	for _, tc := range testCases {
		require.Equal(t, tc.expectedEqual, war.SignersEqualTo(tc.toCompareTo))
	}
}
...
func TestReserveDenomsEqualTo(t *testing.T) {
	...
	war.ReserveTokens = []string{denom1, denom2}
	...
	for _, tc := range testCases {
		...
		require.Equal(t, tc.expectedEqual, war.ReserveDenomsEqualTo(coins))
	}
}

func TestAnyOrderQuantityLimitsExceeded(t *testing.T) {
	war := getValidWar()
	war.OrderQuantityLimits, _ = sdk.ParseCoins("100aaa,200bbb")
	...
	for _, tc := range testCases {
		...
		require.Equal(t, tc.exceedsAnyLimit,
			war.AnyOrderQuantityLimitsExceeded(amounts))
	}
}
...
func TestReservesViolateSanityRateReturnsFalseWhenSanityRateIsZero(t *testing.T) {
	...
	r1 := reserveToken
	r2 := reserveToken2
	war.ReserveTokens = []string{r1, r2}
	...
	for _, tc := range testCases {
		...
		war.SanityRate = srDec
		war.SanityMarginPercentage = smpDec

		actualResult := war.ReservesViolateSanityRate(reserves)
